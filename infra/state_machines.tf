...
resource "aws_iam_role_policy" "sfn" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [
    ...
    { Effect = "Allow", Action = ["states:DescribeExecution"], Resource = "*" }
  ] })
}
  ...
  definition = jsonencode({
    ...
    States = {
      ...
      Consensus = {
        ...
        ResultPath = "$.consensus_raw", Next = "Normalize"
      },
      Normalize = {
        Type = "Task", Resource = "arn:aws:states:::lambda:invoke",
        Parameters = { FunctionName = aws_lambda_function.fn["normalizer"].arn, Payload = { "request.$" = "$", "results.$" = "$.results", "consensus.$" = "$.consensus_raw.Payload" } },
...
resource "aws_sfn_state_machine" "replay" {
  ...
  definition = jsonencode({
    ...
    States = {
      ReplayCorpus = {
        Type = "Map", Label = "ReplayCorpus", MaxConcurrency = 25,
        ItemReader = {
...
resource "aws_cloudwatch_event_rule" "eval" {
  ...
  event_pattern = jsonencode({ source = ["genai.observability.app","genai.observability.replay"], "detail-type" = ["EvaluationRequested"] })
}
...
resource "aws_iam_role_policy" "eventbridge" {
  ...
  policy = jsonencode({ Version = "2012-10-17", Statement = [{ Effect = "Allow", Action = ["states:StartExecution"], Resource = aws_sfn_state_machine.eval.arn }] })
}
resource "aws_cloudwatch_event_target" "sfn" {
  ...
  input_transformer { input_paths = { detail = "$.detail" }
